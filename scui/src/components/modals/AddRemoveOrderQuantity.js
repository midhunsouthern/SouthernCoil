...
import {  getBrazingDetail, setAddBrazingQuantity, setDeleteBrazingQuantity } from "../../constant/url";
...
export default function AddRemoveOrderQuantity(prop) {
  ...
  const handleDesc = (name) =>{
    ...
    return(
        <div className="col">
            <h3 className="text-center"><AddIcon /> / <RemoveIcon /> Quantities @ {orderId+splitId}</h3>
            <p className="text-center">Input the number of Quantity you want to add.</p>
            <div className="col-12 d-flex justify-content-right">
                <div className="col-6">
                    <div className="input-group">
                        <button className="btn secon-bg text-white p-0" onClick={() => handleDesc("qty")}><RemoveIcon fontSize="small" /></button>
                        <input type="text" className="form-control prime-border p-0" value={addQty} name="qty" />
                        <button className="btn secon-bg text-white p-0" onClick={() => handleInc("qty")}><AddIcon fontSize="small" /></button>
                        <button className="btn prime-bg text-white p-0" onClick={() => addQuantitySeries(orderId, splitId, addQty)}>Add Quantity</button>
                    </div>
                </div>
            </div>
            <p className="text-center">Select the Quantity that you what to Remove before spliting</p>
            <div className="row">
            ...
            <div>
              <Button onClick={() => handelDeleteContent(orderId,splitId, childSriesList)} variant="contained" className="mt-2 secon-bg">Delete Selected Series</Button>
            </div>
        </div>
    )
}