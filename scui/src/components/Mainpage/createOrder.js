...
import { Button, TextField, Autocomplete, Checkbox,FormGroup, FormControlLabel, Stack, IconButton, Grid, Box, Typography, ButtonBase, Modal, Avatar, ImageList, ImageListItem, Dialog, DialogActions, FormHelperText, CircularProgress, LinearProgress, styled } from "@mui/material";
...
import { setOrderNew, getCustomersDataAll_dd, getSavedOrderDataByID } from "../../constant/url";
import { AccessContext } from '../../constant/accessContext';
import { orange, blue, lime, green, grey, purple, yellow, red } from '@mui/material/colors';
import { getLookupData, getOrderDataByID, getOrderAll_dd, getLatestOrder } from '../../constant/url'; 
...
import { handleSize, handleSqFeet, handleInput_Check } from "../../commonjs/CommonFun";
...
export default function CreateOrder() {
  ...
  const [isModalCustomerUpdated,setIsModalCustomerUpdated] = React.useState(false);

  const [ hisOrderId,setHisOrderId ] = useState('');
  const [ orderDate,setOrderDate ] = useState(dateObj.getDate()+'/'+(dateObj.getMonth()+1)+'/'+dateObj.getFullYear());
  const [ customerName,setCustomerName ] = useState(0);
  const [ customerNameList, setCustomerNameList ] = useState([{label:'Loading...', id:0}]);
  const [ orderAllList, setOrderAllList ] = useState([{label:'Loading...', id:0}]);
...
//New Pipe Bend components
  ...
  const [ apiCompStatus, setApiCompStatus ] = useState({saveLoading: false, sumbitLoading: false })
  ...
  const handleSubmit = async (type, e) => {
    e.preventDefault();
    setApiCompStatus({...apiCompStatus, sumbitLoading: true, saveLoading:true });
    const imgData = [{ "ep": epPhoto, "assembly": assemblyPhoto, "brazing": brazingPhoto }];
    const btnname = type;
    var bodyFormData = new FormData();
...
//return data
    ...
    setApiCompStatus({...apiCompStatus, sumbitLoading: false, saveLoading:false });
  }
    ...
    axios({
      ...
      .then(function (response) {
        ...
        if(res.status_code === 200){
          ...
          setCustomerName({id:res_data.customer_name, label: res_data.full_customer_name});
    ...
    <div>
          ...
          <ToastContainer />
            <div className="row">
              <div className="col-12">
                    ...
                    <div className="row">
                      ...
                      <div className="col">
                        <TextField
                          ...
                          helperText={moment(orderDate,"DD/MM/YYY").isValid() ? "" : "Data Value is not Correct. Use format DD/MM/YYY" }
                        />
                      </div>
                      <div className="col">
                        <Autocomplete 
                            ...
                            fullWidth
                            renderInput={(params) => <TextField{...params} label="Order History" />}  
                            onChange={(e, value) => {handleGetOrderById( value);}}
                            value={hisOrderId}
                            key={"orderid-1"}
                          />
                      </div>
                      <div className="col">
                        <Button className="prime-bg" variant="contained"  onClick={handleOpen}>Display Saved Order</Button>
                      </div>
                    </div>
                  </div>
                </div>
                          ...
                          <div className="col-12">
                            <div className="row  mt-2 rounded">
                              <div className="row  mt-2 rounded prime-border p-2 m-1" >
                                <div className="col-6">
                                  <Stack direction="row" spacing={2}>
                                    <Button className="prime-bg " variant="contained" data-bs-toggle="modal" data-bs-target="#staticBackdrop" sx={{height:"60px"}}>
                                      <PersonAddAltIcon />
                                    </Button>
                                    <Autocomplete 
                                      disableClearable
                                      ...
                                      getOptionLabel={(customerNameList) => customerNameList.label || ""}
                                      getOptionSelected={(option, value) => option.id === value.id}
                                      isOptionEqualToValue={(option, value) => option.id === value.id}
                                      sx={{ width: 300 }}
                                      renderInput={(params) => <TextField{...params} label="Customers" required
                                        error={customerName === 0}
                                        helperText={customerName === 0 ? "Select Customer" :"" } />}
                                      fullWidth   
                                      onChange={(e, value) => {setCustomerName({label: value.label, id:value.id});}}
                                      value={customerName}
                                      key={"custname"+1}
                                    />
                                ...
                                <div className="col-3">
                                  <TextField
                                    ...
                                    helperText={length.length === 0 ? "Enter Proper Value." : "" }
                                  />
                                </div>
                                <div className="col-3">
                                  <TextField
                                    ...
                                    helperText={height.length === 0 ? "Enter Proper Value." : "" }
                                  /> 
                                </div>
                                <div className="col-3">
                                  <TextField
                                ...
                                <div className="col-3">
                                  <TextField
                                    ...
                                    value={quantity}
                                    onChange={(e) =>{e.target.value >99 ? setQuantity(99): setQuantity(e.target.value)}}
                                    error={quantity.length === 0}
                                    helperText={quantity.length === 0 ? "Enter Proper Value." : "" }
                                  />
                                </div>
                              </div>
                              <div className="row  mt-2 rounded prime-border p-2 m-1" >
                                <div className="col-3">
                                  <FormControl fullWidth>
                                      ...
                                      >
                                        {
                                          lookUpList['pipeType']?.map((item) => {return(
                                ...
                                <div className="col-3">
                                <FormControl fullWidth>
                                      <InputLabel id="expansionType-label">Expansion Type</InputLabel>
                                      <Select
                                        required={true}
                                      ...
                                      >
                                        {
                                          lookUpList['expansionType']?.map((item) => {return(
                                              <MenuItem value={item.id}>{item.lkp_value}</MenuItem>
                                          )}
                                          )
                                        }
                                      ...
                                      {expansionType === '' &&
                                        <FormHelperText error>Select Expansion Type</FormHelperText>
                                      }
                                    </FormControl>
                                </div>
                                <div className="col-6">
                                  <TextField
                                    ...
                                    value={pipeComment}
                                    onChange={(e) => {setPipeComment(e.target.value); }}
                                  />
                                </div>
                                <div className="col-12">
                                <InputLabel >Pipe Bend Data</InputLabel>
                                {(pbStraight !== "true" && pbSingle !== "true" && pbCross !== "true" && pbOther !== "true" ) &&
                                            <FormHelperText error>Select any of the Pipe Bend</FormHelperText>
                                          }
                                      <Stack direction="row" spacing={{ xs: 1, sm: 6, md: 12 }} alignContent="space-between">
                                        <Stack  >
                                          <FormControlLabel  control={<Checkbox value="Straight"  onChange={(event) =>setPBStraight(event.target.checked.toString())}  checked={(pbStraight === 'true')} name="pipebend"/>} label="Straight" />
                                          ...
                                          <Stack>
                                              ...
                                              type="number"
                                                ...
                                                value={pbStraightQty}
                                                onChange={(e) => {setPBStraightQty( e.target.value); }}
                                              />
                                              <TextField
                                                InputLabelProps={{ shrink: true }}
                                              type="number"
                                              ...
                                              onChange={(e) => {setPBStraightSize( e.target.value); }}
                                              />
                                              <TextField
                                              InputLabelProps={{ shrink: true }}
                                              type="number"
                                        ...
                                        <Stack >
                                          <FormControlLabel  control={<Checkbox value="Single"  onChange={(event) => setPBSingle( event.target.checked.toString())} 
                                          checked={ (pbSingle === 'true') }/>} label="Single" name="pipebend" />
                                         {
                                          (pbSingle === 'true') && 
                                          <Stack>
                                            <TextField
                                              ...
                                              value={pbSingleQty}
                                              onChange={(e) => {setPBSingleQty( e.target.value); }}
                                            />
                                            <TextField
                                              InputLabelProps={{ shrink: true }}
                                            type="number"
                                              ...
                                              value={pbSingleSize}
                                              onChange={(e) => {setPBSingleSize( e.target.value); }}
                                            />
                                            <TextField
                                              InputLabelProps={{ shrink: true }}
                                            type="number"
                                        ...
                                        <Stack>
                                          <FormControlLabel  control={<Checkbox value="Cross"  onChange={(event) => setPBCross(event.target.checked.toString())} 
                                          checked={ (pbCross === 'true') } />} label="Cross" name="pipebend" />
                                          {
                                            (pbCross === 'true') && <Stack>
                                              <TextField
                                            InputLabelProps={{ shrink: true }}
                                            ...
                                            value={pbCrossQty}
                                            onChange={(e) => {setPBCrossQty( e.target.value ); }}
                                          />
                                          <TextField
                                            InputLabelProps={{ shrink: true }}
                                            type="number"
                                            ...
                                            value={pbCrossSize}
                                            onChange={(e) => {setPBCrossSize( e.target.value); }}
                                          />
                                          <TextField
                                            InputLabelProps={{ shrink: true }}
                                          type="number"
                                        ...
                                        <Stack >
                                          <FormControlLabel  control={<Checkbox value="Other"  
                                          onChange={(event) => setPBOther(event.target.checked.toString())} checked={ (pbOther === 'true')}/>} label="Other" />
                                          {
                                            (pbOther === 'true') && <Stack>
                                            <TextField
                                              InputLabelProps={{ shrink: true }}
                                              ...
                                              value={pbOtherQty}
                                              onChange={(e) => {setPBOtherQty( e.target.value);  }}
                                            />
                                            <TextField
                                              InputLabelProps={{ shrink: true }}
                                              type="number"
                                              ...
                                              value={pbOtherSize}
                                              onChange={(e) => {setPBOtherSize( e.target.value); }}
                                            />
                                            <TextField
                                              InputLabelProps={{ shrink: true }}
                                              type="number"
                                              ...
                                              value={pbOtherTotQty}
                                              onChange={(e) => {setPBOtherTotQty( e.target.value); }}
                                            />
                                            </Stack>
                                          }
                              ...
                              <div className="row  mt-2 rounded prime-border p-2" >
                                <div className="col-3">
                                <FormControl fullWidth>
                                      <InputLabel id="modeDispatch-label">End Plate Material</InputLabel>
                                      <Select
                                        required={true}
                                        labelId="modeDispatch-label"
                                        ...
                                        onChange={(event)=> setEndPlateMaterial(event.target.value)}
                                        >
                                          {
                                            lookUpList['epMaterial']?.map((item) => {return(
                                                <MenuItem value={item.id}>{item.lkp_value}</MenuItem>
                                            )}
                                            )
                                          }
                                      ...
                                      {endPlateMaterial.length <=0 &&
                                        <FormHelperText error>Select End Plate Material</FormHelperText>
                                      }
                                    </FormControl>
                                </div>
                                <div className="col-3">
                                  <FormControl fullWidth>
                                      <InputLabel id="modeDispatch-label">End Plate Modal</InputLabel>
                                      <Select
                                        required={true}
                                        labelId="modeDispatch-label"
                                        id="epModel-select"
                                        ...
                                        value={endPlateModel}
                                        onChange={(event)=> setEndPlateModel(event.target.value)}
                                        >
                                          {
                                            lookUpList['epModal']?.map((item) => {return(
                                                <MenuItem value={item.id}>{item.lkp_value}</MenuItem>
                                            )}
                                            )
                                          }
                                      ...
                                      {endPlateModel?.length <=0 &&
                                        <FormHelperText error>Select End Plate Model</FormHelperText>
                                      }
                                    </FormControl>
                                </div>
                                <div className="col-3">
                                  ...
                                  <>
                                  {
                                    ...
                                    <ReactFileReader
                                      fileTypes={[".png", ".jpg"]}
                                      base64={true}
                                      handleFiles={(files) => {handleFiles('ep',files)}}
                                      multipleFiles={true}
                                      key={Math.random()}
                                    >
                                        <PhotoCameraIcon />
                                ...
                                <Stack direction="row" spacing={1}>
                                  { 
                                      lookUpList['oreientation']?.map((item) => {return( 
                                        <>
                                        <FormControlLabel  control={ 
                                        <Checkbox value={item.id}   key={Math.random()}
                                        onChange={(event) => { setEndPlateOrientation(handleInput_Check(endPlateOrientation,event.target.value,event.target.checked)); setChkst(Math.random());}} 
                                        checked={endPlateOrientation.indexOf(item.id) !== -1}
                                      />
                                ...
                                <div className="col-12">
                                  ...
                                  <FormHelperText error>Select Cover Type-Details</FormHelperText>
                                }
                                    <Stack direction="column" spacing={2} >
                                  { chkst > 0 && 
                                    lookUpList['coverType']?.map((item) => {
                                      return(
                                      <div style={{backgroundColor: '#D06800', padding:"3px", borderRadius:"5px"}}>
                                        <InputLabel id="cover-label" style={{color:"white"}}>{item.lkp_value}</InputLabel>
                                        <Stack direction="row" style={{backgroundColor: "#F37C06", padding:"3px", borderRadius:"5px", overflow:"scroll"}}>
                                          {lookUpList['coverDetail']?.map((itemd) => {
                                              if (itemd.lkp_id === item.id){
                                                return(
                                                  <FormControlLabel  control={<Checkbox value={itemd.id} 
                                                  checked={coverDetail.indexOf(itemd.id) !== -1} 
                                                  onChange={(event) => { setCoverDetail(handleInput_Check(coverDetail,event.target.value,event.target.checked)); setChkst(Math.random())}} 
                                ...
                                <div className="col-6">
                                  <TextField
                                    ...
                                    value={epComments}
                                    onChange={(e) => {setEPComments(e.target.value); }}
                                  />
                                </div>
                              </div>
                              <div className="row  mt-2 rounded prime-border p-2"  >
                                <div className="col-4">
                                  <TextField
                                    ...
                                    helperText={ finPerInch.length <= 0 ? "Enter Fin Per Inch":""}
                                  />
                                </div>
                                <div className="col-4">
                                  <TextField
                                    ...
                                    value={finComments}
                                    onChange={(e)=>{setFinComments(e.target.value); }}
                                  />
                                </div>
                                <div className="col-4">
                                  <InputLabel>Photo upload Hair pin(Assembly)</InputLabel>
                                  { assemblyPhoto.length === 0 &&
                                        <FormHelperText error>Select Assembly Photo</FormHelperText>
                                      }
                                  <>
                                  {
                                    <ImageList  cols={3} rowHeight={164}>
                                    {assemblyPhoto?.map((item, index) => (
                                      <ImageListItem key={'assemblyphoto'+index}>
                                        <img
                                            src={item}
                                            srcSet={item}
                                            alt={'assemblyphoto'}
                              ...
                              <div className="row mt-2 rounded prime-border p-2" >
                                <div className="col-3">
                                  <FormControl fullWidth>
                                      <InputLabel id="circuitmodel-label">Circuit Model</InputLabel>
                                      <Select
                                        required={true}
                                        ...
                                        label="Circuit Model"
                                        value={circuitModels}
                                        onChange={(event)=> setCircuitModels(event.target.value)}
                                        >
                                          {
                                            lookUpList['circuitModel']?.map((item) => {return(
                                                <MenuItem value={item.id}>{item.lkp_value}</MenuItem>
                                            )}
                                            )
                                          }
                                      ...
                                      { circuitModels === '' &&
                                        <FormHelperText error>Select Circuit Models</FormHelperText>
                                      }
                                    </FormControl>
                                </div>
                                <div className="col-3">
                                  <TextField
                                      ...
                                      helperText={ noCircuit.length <= 0 ? "Enter No of Circuit":""}
                                    />
                                </div>
                                <div className="col-3">
                                  <TextField
                                      ...
                                      onChange={(e) => {setBrazingComments(e.target.value); }}
                                    />
                                </div>
                                <div className="col-3">
                                ...
                                <div className="col-8">
                                      ...
                                      {
                                        lookUpList['liquidLine']?.map((item) => {return(
                                          <FormControlLabel  control={<Checkbox value={item.id}
                                           onChange={(event) => {setLiquidLine(handleInput_Check(liquidLine,event.target.value , event.target.checked)); setChkst(Math.random());}} 
                                           checked={liquidLine.indexOf(item.id) !== -1}   key={Math.random()}
                                          />} label={item.lkp_value} />
                                        )}
                                        )
                                      }
                                ...
                                <div className="col-4">
                                      <InputLabel id="dischargeLine-label">Discharge Line</InputLabel>
                                      { dischargeLine.length <= 0 &&
                                        <FormHelperText error>Select Discharge Line</FormHelperText>
                                      }
                                      <Stack direction="row" spacing={0} useFlexGap flexWrap="wrap">
                                      {
                                        lookUpList['dischargeLine']?.map((item) => {return(
                                          <FormControlLabel  control={<Checkbox value={item.id} checked={dischargeLine.indexOf(item.id) !== -1} 
                                          onChange={(event) => {setDischargeLine(handleInput_Check(dischargeLine,event.target.value , event.target.checked)); setChkst(Math.random());}} 
                                          />} label={item.lkp_value} />
                                        )}
                                        )
                                      }
                              ...
                              <div className="row  mt-2 rounded prime-border p-2" >
                                <div className="col-6">
                                      <InputLabel id="PaintingType-label">Painting Type</InputLabel>
                                      {  paintType.length <= 0 &&
                                        <FormHelperText error>Select Painting Type</FormHelperText>
                                      }
                                      <Stack direction="row" spacing={0} useFlexGap flexWrap="wrap">
                                      {
                                        lookUpList['paintType']?.map((item) => {return(
                                          <FormControlLabel  control={<Checkbox value={item.id}  checked={paintType.indexOf(item.id) !== -1}  
                                          onChange={(event) => {setPaintType( handleInput_Check(paintType,event.target.value , event.target.checked)); setChkst(Math.random); }} 
                                          />} label={item.lkp_value} />
                                        )}
                                        )
                                      }
                                  ...
                                  <div className="col-6">
                                    <InputLabel id="modeDispatch-label">Mode of Dispatch</InputLabel>
                                    {dispatchMode.length <= 0 &&
                                        <FormHelperText error>Select Mode of Dispatch</FormHelperText>
                                      }
                                    <Stack direction="row" spacing={0} useFlexGap flexWrap="wrap">
                                      {
                                        lookUpList['dispatchMode']?.map((item) => {
                                          ...
                                          onChange={(event) => {setDispatchMode(handleInput_Check(dispatchMode,event.target.value , event.target.checked)); setChkst(Math.random);}} 
                                          />} label={item.lkp_value} />
                                        )}
                                        )
                                      }
                                  ...
                                  <div className="col-6">
                                      ...
                                      { packingType.length <= 0 &&
                                        <FormHelperText error>Select Packing Type</FormHelperText>
                                      }
                                      <Stack direction="row" spacing={0} useFlexGap flexWrap="wrap">
                                  {
                                    lookUpList['packingType']?.map((item) => {return(
                                      <FormControlLabel  control={<Checkbox value={item.id} checked={packingType.indexOf(item.id) !== -1} 
                                      onChange={(event) => {setPackingType(handleInput_Check(packingType,event.target.value , event.target.checked)); setChkst(Math.random);}} 
                                        />} label={item.lkp_value} />
                                    )}
                                    )
                                  }
                ...
                >
                  <Box sx={style}>
                    <SavedTempOrders  selectedRow = {(retId) => handleSelectedOrder(retId)}/>
                  </Box>
                </Modal>
                </div>
              </div>
      ...
      >
        <div className="col p-5">
          <Typography variant="h4">Your Order is Placed Successfully.</Typography>
          <Typography variant="h4">Order Id is: {retOrderId}</Typography>
          <br/>
          <Typography variant="p">Please Navigate to the order page to view the full Order.</Typography>
        </div>
        <DialogActions>
          <Button onClick={() => handleCloseOrderStatusDialog('yes')}>Close</Button>
        </DialogActions>
      </Dialog>
    </div>
  );
