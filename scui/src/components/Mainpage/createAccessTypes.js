...
import { getModuleList, getAccessNameList, setAccessModuleList, getExistingAccessModuleList } from '../../constant/url';
              ...
              >
                ...
                <TableCell>
                  <RadioGroup name="use-radio-group" defaultValue="1" row  >
                    <Radio checked={row.access_rw === '1'} value={'1'} onChange={() => handleModuleAccessType(row.id,1)} />
                    <Radio checked={row.access_rw === '2'} value={'2'} onChange={() => handleModuleAccessType(row.id,2)} />
                    <Radio checked={row.access_rw === '0'} value={'0'} onChange={() => handleModuleAccessType(row.id,0)} />
                  </RadioGroup>
                </TableCell>
              </TableRow>
            ))}
                              ...
                              <div className="row">
                                <div className="col">
                                  <Autocomplete
                                    ...
                                    sx={{ width: 300 }}
                                    renderInput={(params) => <TextField {...params} label="Access Types Existing" />}
                                    fullWidth
                                    
                                    onChange={(e,value) => handleSelectAccessType(value)}
                                  />
                                </div>
                                <div className="col">
                                  <TextField id="outlined-basic" label="Access Types New" variant="outlined" value={accessName}  onChange={(e) => {setAccessName(e.target.value)}}/>
                                </div>
                              </div>
                                <ModuleComponent/>
                              </div>
                              <div className="float-right my-2 ">
                                <Button variant="contained" size="large" type="submit" onClick={() => handleSetModuleList()}>
                                  Save
                                </Button>
                              </div>
                            </Container>
