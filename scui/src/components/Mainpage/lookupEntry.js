...
import { Container, TextField, Card, Button, TableContainer, Table, TableHead, TableCell, TableBody, TableRow, FormControl, InputLabel, Select, MenuItem, Modal } from '@mui/material';
import axios from 'axios';
import { AccessContext } from '../../constant/accessContext';
import { getLookupData, setLookupData, delLookupData } from '../../constant/url'; 
      ...
      return(
          <Card id={"card"+index}>
              <div id={"main"+index}>
                  ...
                  <div id={"body-div"+index} className='row'>{ (lkpCat !== 'coverDetail') &&
                      <div id={"lkuvaluediv"+index} className='col-4'>
                          <TextField label="Look up Value" key={"lkpvalue"+index }
                                  name="lkpvalue"
                                  autoComplete="lkpvalue" required fullWidth  value={value} onChange={(e) => {setValue(e.target.value);}} />
                      </div>
                      }   
                      {(lkpCat === 'coverDetail') &&
                      <div id={"divrow "+index} className='row'>
                          ...
                          <FormControl fullWidth>
                            ...
                            <Select 
                              ...
                              >
                                {
                                  lookUpList['coverType']?.map((item) => {return(
                          ...
                          <div className='col-4'>
                              <TextField label="Sub Look up Value" key={"sublkpvalue" + index}
                                      name="lkpvalue"
                                      autoComplete="lkpvalue" required fullWidth  value={subValue} onChange={(e) => {setSubValue(e.target.value);}} />
                          </div>
                      </div>
                      
                      }   
                      <div className='col-4'>
                          <Button type='submit' key={"btn"+index } variant="contained" id={"button"+index} onClick={() => handleSubmit(value, subValue)}>Update</Button>
                      </div>
                  </div>
                  <div className='mt-2'>
                      <TableContainer>
                                  ...
                                  {lkpList?.map( (item) => {
                                      return(
                                          <TableRow>
                                              ...
                                              <TableCell>
                                              <Tooltip title="Edit" id={"tooledit"+index+'-'+item.id} color='info'>
                                                  <IconButton id={"edit-button"+index+'-'+item.id} data-bs-toggle="modal" data-bs-target="#staticBackdrop"  key={"icbtnedit"+index }
                                                  onClick={() => {setSelectedRowId(item.id); setEditValue(item.lkp_value); setEditSubValue(item.sublkp_val)}}>
                                                      <CreateRoundedIcon/>
                                                  </IconButton>
                                              </Tooltip>
                                              <Tooltip title="Delete" id={"tooldelete"+index+'-'+item.id}>
                                                  <IconButton  color='error' onClick={() => handleDelete(item.id)} key={"icbtndel"+index }>
                                                      <DeleteIcon />
                                                  </IconButton>
                                              </Tooltip>
                                              </TableCell>
    ...
    <ToastContainer />
        ...
        <AppBar position="static">
            ...
            >
                ...
                <Tab label="Expansion Type" name="expansionType" {...a11yProps(1)}/>
                {/*<Tab label="Pipe Bend" name="pipeBend" {...a11yProps(2)}/>*/}
                <Tab label="End Plate Material" name="epMaterial" {...a11yProps(2)}/>
                ...
                <Tab label="Circuit Model" name="circuitModel" {...a11yProps(7)}/>
                <Tab label="Liquid Line" name="liquidLine" {...a11yProps(8)}/>
                <Tab label="Discharge Line" name="dischargeLine" {...a11yProps(9)}/>
                <Tab label="Paint Type" name="paintType" {...a11yProps(10)}/>
                <Tab label="Packing Type" name="packingType" {...a11yProps(11)} />
                <Tab label="Dispatch Mode" name="dispatchMode" {...a11yProps(12)}/>
                <Tab label="Brazing Lookup" name="brazingLkp" {...a11yProps(13)}/>
            </Tabs>
        </AppBar>
        <SwipeableViews
        ...
        >
          ...
          <div className="modal modal-lg fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false"  aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div className="modal-dialog">
                <div className="modal-content">
                    <div className="modal-header">
                        <h5 className="modal-headline" id="staticBackdropLabel">Edit Look Up Data</h5>
                    ...
                    <div className="modal-body">
                        <Container>
                            <Card>
                                ...
                                <div className='row'>{ (lkpCat !== 'coverDetail') &&
                                    <div className='col-4'>
                                        <TextField label="Look up Value" id="lkpvalue" 
                                                name="lkpvalue"
                                                autoComplete="lkpvalue" required fullWidth  value={editValue} onChange={(e) => {setEditValue(e.target.value);}} />
                                    </div>
                                    }   
                                    {(lkpCat === 'coverDetail') &&
                                    <div className='row'>
                                        ...
                                        <div className='col-4'>
                                            <TextField label="Sub Look up Value" id="lkpvalue" 
                                                    name="lkpvalue"
                                                    autoComplete="lkpvalue" required fullWidth value={editSubValue}  onChange={(e) => setEditSubValue(e.target.value)} />
                                        </div>
                                    </div>
                                    
                                    }   
                                    <div className='col-4'>
                                        <Button type='submit' variant="contained" onClick={() => handleSubmit(editValue,editSubValue,selectedRowId)}>Update</Button>
                                    </div>
                                </div>
                            </Card>
                        </Container>
